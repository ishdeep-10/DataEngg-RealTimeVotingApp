...
def plot_colored_bar_chart(results):
    ...
    plt.title('Vote Counts per Candidate',fontsize=30)
    plt.xticks(rotation=90,fontsize=15)
    plt.gca().patch.set_facecolor('black')
    # Customize the text and ticks color to be visible against the black background
...
def plot_donut_chart(data: pd.DataFrame, title='Donut Chart', type='candidate'):
    ...
    # Set title with increased size
    plt.title(title, fontsize=50, color='white')
    
    # Improve the visibility of the text on the chart
...
def update_data():
    ...
    last_refresh.text(f"Last Refesh at: {time.strftime('%Y-%m-%d %H-%M-%S')}")
